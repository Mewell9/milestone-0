...
This monorepo already has a FastAPI app at `services/api/` (`models.py` is suppliers; `get_db()` is TinyDB). Locked mapping:

```text
services/api/
├── main.py                          # one FastAPI app
├── database.py                      # TinyDB get_db() + re-exports get_inventory_session
├── app/inventory/models.py          # Ingredient, IngredientEntry, IngredientExit
├── app/inventory/schemas.py         # Pydantic only (not ORM classes)
├── app/inventory/router.py          # APIRouter(prefix="/inventory")
└── app/routers/inventory.py         # thin register path for main.py
```

## Locked decisions (this repo)

1. **Layout:** Keep inventory in `app/inventory/`. Register via `app/routers/inventory.py`. Do not flatten into supplier `models.py`. Do not add a second FastAPI app.
2. **Stock:** `current_stock` is chain-wide per ingredient: `SUM(entries) − SUM(exits)`. `location_id` is stored on orders only; it is not a stock partition.
